// $Id: mbng_matrix.c 1944 2014-01-28 21:24:35Z tk $
...
# error "not prepared for != 16 rows - id assignments have to be adapted!"
  ...
  if( !item->flags.led_matrix_pattern ) {
	...
	MBNG_MATRIX_DOUT_PatternSet_Digit(matrix, item->flags.colour, row, value, NUM_MATRIX_DIM_LEVELS-1, dot);
      ...
      //digit_blankzero and digit_signed added by Sauraen
      u32 value = item->value;
      u8 negative = 0;
      u32 divisor = 1;
      if(item->flags.digit_signed && value >= 0x8000){
        value = 0x10000 - value;
        negative = 1;
      }
      //Signed puts negative sign where the first blanked digit was, so it makes sense to imply it
      u8 blankzero = item->flags.digit_blankzero || item->flags.digit_signed;
      switch( item->flags.led_matrix_pattern ) {
      case MBNG_EVENT_LED_MATRIX_PATTERN_DIGIT1: blankzero = 0; break;
      case MBNG_EVENT_LED_MATRIX_PATTERN_DIGIT2: divisor = 10; break;
      case MBNG_EVENT_LED_MATRIX_PATTERN_DIGIT3: divisor = 100; break;
      case MBNG_EVENT_LED_MATRIX_PATTERN_DIGIT4: divisor = 1000; break;
      case MBNG_EVENT_LED_MATRIX_PATTERN_DIGIT5: divisor = 10000; break;
      //case MBNG_EVENT_LED_MATRIX_PATTERN_NEGATIVESIGN: //doesn't matter
      }
      if(negative && value < divisor && value >= (divisor/10)){
        value = 0x2D;
      }else{
        value /= divisor;
        if(value >= 10) blankzero = 0;
        value %= 10;
        if(value == 0 && blankzero){
          value = 0;
        }else{
          value += 0x30; // number ASCII codes
        }
      }
      u8 dot = 0;
