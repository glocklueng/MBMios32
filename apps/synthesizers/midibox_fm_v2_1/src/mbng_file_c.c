// $Id: mbng_file_c.c 2046 2014-09-04 19:52:54Z tk $
...
#define LABEL_MAX_SIZE 41
  ...
  while( parseExtendedParameter(line, cmd, &parameter, &value_str, &brkt) >= 0 ) { 
    ...
    //XXX is this right? max size plus init values?
	  item.stream_size = 2;
	  item.stream[0] = 0;
	  item.stream[1] = 0;
	} break;  
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////
    } else if( strcasecmp(parameter, "mbfm") == 0 ) {
      if( item.flags.type != MBNG_EVENT_TYPE_MBFM ) {
        #if DEBUG_VERBOSE_LEVEL >= 1
	        DEBUG_MSG("[MBNG_FILE_C:%d] ERROR: mbfm is only expected for MBFM events in EVENT_%s ... %s=%s\n", line, event, parameter, value_str);
        #endif
	      return -1;
      }
      ...
      mbng_event_mbfm_type_t mbfm_type;
      if( !(values_str = strtok_r(value_str, separator_colon, &brkt_local)) ||
	          (mbfm_type = MBNG_EVENT_ItemMBFMTypeFromStrGet(values_str)) == MBNG_EVENT_MBFM_TYPE_UNDEFINED ) {
        #if DEBUG_VERBOSE_LEVEL >= 1
	        DEBUG_MSG("[MBNG_FILE_C:%d] ERROR: invalid mbfm type in EVENT_%s ... %s=%s\n", line, event, parameter, value_str);
        #endif
	      return -1;
      }

      item.stream[item.stream_size++] = mbfm_type;
      ...
      for(i=0; i<num_bytes; ++i) {
	      ...
	      if( !(values_str = strtok_r(NULL, separator_colon, &brkt_local)) ||
	          (value=get_dec(values_str)) < 0 || value > 255 ) {
          #if DEBUG_VERBOSE_LEVEL >= 1
	          DEBUG_MSG("[MBNG_FILE_C:%d] ERROR: expecting %d values for mbfm type in EVENT_%s ... %s=%s\n", line, num_bytes, event, parameter, value_str);
          #endif
	        return -1;
	      }
    ...
    } else if( strcasecmp(parameter, "digit_blankzero") == 0 ) {
      int value;
      if( (value=get_dec(value_str)) < 0 || value > 1 ) {
#if DEBUG_VERBOSE_LEVEL >= 1
	DEBUG_MSG("[MBNG_FILE_C:%d] ERROR: invalid flag in EVENT_%s ... %s=%s (expect 0 or 1)\n", line, event, parameter, value_str);
	...
	return -1;
      ...
      item.flags.digit_blankzero = value;
    
    } else if( strcasecmp(parameter, "digit_signed") == 0 ) {
      int value;
      if( (value=get_dec(value_str)) < 0 || value > 1 ) {
#if DEBUG_VERBOSE_LEVEL >= 1
	DEBUG_MSG("[MBNG_FILE_C:%d] ERROR: invalid flag in EVENT_%s ... %s=%s (expect 0 or 1)\n", line, event, parameter, value_str);
	...
	for(i=0; i<item.stream_size; ++i) {
      ...
      case MBNG_EVENT_TYPE_MBFM: {
	      ...
	      for(i=0; i<item.stream_size; ++i) {
	        ...
	        sprintf(line_buffer, " mbfm=%s", MBNG_EVENT_ItemMBFMTypeStrGet(mbfm_type));
	        FLUSH_BUFFER;
	        ...
	        for(j=0; j<num_bytes; ++j) {
	          ...
	          sprintf(line_buffer, ":%d", (int)mbfm_value);
	          FLUSH_BUFFER;
	        }
	      }
      } break;
