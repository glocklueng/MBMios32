// $Id: app.c 366 2009-02-24 08:42:03Z tk $
/*
 *
...
struct telnet_state *telnet_client_state;
...
void APP_MIDI_NotifyPackage(mios32_midi_port_t port, mios32_midi_package_t midi_package)
{
    ...
    if (midi_package.evnt1 == 0x5f
     && midi_package.evnt2 > 0) {
#if TN_DEBUG_VERBOSE_LEVEL >= 1
         MIOS32_MIDI_SendDebugMessage("Connection request sending now");
#endif
         telnet_client_state = telnet_connect(10,0,0,1,23);
     }
    // 2nd last key sends the letter 's' to the server
    if (midi_package.evnt1 == 0x5e
     && midi_package.evnt2 > 0) {
#if TN_DEBUG_VERBOSE_LEVEL >= 1
         MIOS32_MIDI_SendDebugMessage("Trying to send the letter s");
#endif
         telnet_sendstring(telnet_client_state, "s");
     }
