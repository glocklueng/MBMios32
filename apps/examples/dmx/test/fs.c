...
/////////////////////////////////////////////////////////////////////////////
char *FS_FindNext(char *filename)
{
#if DEBUG_VERBOSE_LEVEL >= 1
  DEBUG_MSG("[FS] Opening directory '%s'\n", FS_FILES_PATH);
#endif

  di.scratch = sector;
  if( DFS_OpenDir(&vi, FS_FILES_PATH, &di) ) {
#if DEBUG_VERBOSE_LEVEL >= 1
    DEBUG_MSG("[FS] Error opening directory - try mounting the partition again\n");
#endif

    s32 error;
    if( (error =FS_mount_fs()) < 0 ) {
#if DEBUG_VERBOSE_LEVEL >= 1
      DEBUG_MSG("[FS] mounting failed with status: %d\n", error);
#endif
      strcpy(ui_file_name, "no SD-Card");
  ...
  while( !DFS_GetNext(&vi, &di, &de) ) {
    if( de.name[0] ) {
      DFS_DirToCanonical(ui_file_name, de.name);
#if DEBUG_VERBOSE_LEVEL >= 2
      DEBUG_MSG("[FS] file: '%s'\n", ui_file_name);
#endif
	...
	if( take_next ) {
#if DEBUG_VERBOSE_LEVEL >= 2
	  DEBUG_MSG("[FS] return it as next file\n");
#endif
	  return ui_file_name;
	} else if( strncasecmp(&de.name[0], (char *)search_file, 8) == 0 ) {
#if DEBUG_VERBOSE_LEVEL >= 2
	  DEBUG_MSG("[FS] found file: '%s', searching for next\n", ui_file_name);
#endif
	  take_next = 1;
	}
      }
...
#endif
    strcpy(ui_file_name, "file error");
    file_len = 0;
    return -1;
  }
...
/////////////////////////////////////////////////////////////////////////////
u32 FS_read(void *buffer, u32 len)
{
  u32 successcount;
  ...
  if( successcount != len ) {
#if DEBUG_VERBOSE_LEVEL >= 1
    DEBUG_MSG("[FS] unexpected read error - DFS cannot access offset %u (len: %u, got: %u)\n", file_pos, len, successcount);
#endif
  }
  
  file_pos += len; // no error, just failsafe: hold the reference file position, regardless of successcount
