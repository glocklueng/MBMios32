// $Id: seq_file_m.c 1387 2011-12-29 17:49:56Z tk $
/*
...
// returns < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 SEQ_FILE_M_SaveAllBanks(char *session)
{
  s32 status = SEQ_FILE_M_MapWrite(session, SEQ_MIXER_NumGet(), 0);

#if DEBUG_VERBOSE_LEVEL >= 1
  DEBUG_MSG("[SEQ_FILE_M] Tried to store mixer map, status: %d\n", status);
#endif
...
s32 SEQ_FILE_M_Create(char *session)
{
  ...
  status |= FILE_WriteHWord(info->header.num_maps);
  ...
  status |= FILE_WriteHWord(info->header.map_size);
...
s32 SEQ_FILE_M_Open(char *session)
{
  ...
  status |= FILE_ReadHWord((u16 *)&info->header.num_maps);
  status |= FILE_ReadHWord((u16 *)&info->header.map_size);
...
s32 SEQ_FILE_M_MapWrite(char *session, u8 map, u8 rename_if_empty_name)
{
  ...
  for(chn=0; chn<num_chn; ++chn) {
    u8 par;
    for(par=0; par<num_par; ++par)
      status |= FILE_WriteByte(SEQ_MIXER_Get(chn, par));
  }
