// $Id: mm_vpot.c 1491 2012-07-29 20:41:15Z tk $
/*
 * MIDIbox MM V3
 * VPot Handler
 *
...
// MM_VPOT_LEDRingHandler seperately if the V-Pot pointer hasn't been received from host
...
s32 MM_VPOT_Init(u32 mode)
{
  ...
  for(i=0; i<MM_HWCFG_ENC_NUM; ++i)
    MIOS32_ENC_ConfigSet(i, mm_hwcfg_encoders[i]);
...
/////////////////////////////////////////////////////////////////////////////
s32 MM_VPOT_ValueSet(u8 vpot, u8 value)
{
  ...
  //MM_LCD_Update_Knob(vpot); // request update on LCD
...
s32 MM_VPOT_SendENCEvent(u8 encoder, s32 incrementer)
{
  if( mm_hwcfg_verbose_level >= 2 ) {
    MIOS32_MIDI_SendDebugMessage("  -> MM_VPOT_SendEncEvent(%d, %d)\n", encoder, incrementer);
  }
  ...
  if( mm_flags.GPC_SEL ) {
    ...
    u8 cc_number = 0x40 | (encoder & 0x07);

    u8 value;
    if( incrementer < 0 ) {
      value = (-incrementer & 0x3f) | 0x00; // anti-clockwise
    } else {
      value = (incrementer & 0x3f) | 0x40; // clockwise
    }

    MIOS32_MIDI_SendCC(DEFAULT, (mm_hwcfg_midi_channel-1) & 0x0f, cc_number, value);
  }
...
{
  if( mm_hwcfg_verbose_level >= 2 ) {
    MIOS32_MIDI_SendDebugMessage("  -> MM_VPOT_SendJogWheelEvent(%d)\n", incrementer);
  }
  ...
  if( mm_flags.GPC_SEL ) {
    ...
    MIOS32_MIDI_SendCC(DEFAULT, chn, cc_number, value);
  }
...
s32 MM_VPOT_LEDRing_SRHandler(void)
{
  ...
  if( mm_hwcfg_ledrings.cathodes_ledrings_sr == 0 && mm_hwcfg_ledrings.cathodes_meters_sr == 0 ) {
    return 0; // nothing to do here...
  } else if( mm_hwcfg_ledrings.cathodes_ledrings_sr && mm_hwcfg_ledrings.cathodes_meters_sr == 0 ) {
    ...
    if( mm_hwcfg_ledrings.cathodes_ledrings_sr )
      MIOS32_DOUT_SRSet(mm_hwcfg_ledrings.cathodes_ledrings_sr-1, select_mask);

    // set the LEDring pattern on the anodes
    //anode_pattern = mm_flags.LEDRING_METERS ? meter_pattern[sr_ctr] : ledring_pattern[sr_ctr];
    anode_pattern = ledring_pattern[sr_ctr];

  } else if( mm_hwcfg_ledrings.cathodes_ledrings_sr == 0 && mm_hwcfg_ledrings.cathodes_meters_sr ) {
    ...
    if( mm_hwcfg_ledrings.cathodes_meters_sr )
      MIOS32_DOUT_SRSet(mm_hwcfg_ledrings.cathodes_meters_sr-1, select_mask);

    // set the meter pattern on the anodes
    //anode_pattern = mm_flags.LEDRING_METERS ? ledring_pattern[sr_ctr] : meter_pattern[sr_ctr];
    anode_pattern = ledring_pattern[sr_ctr];
	...
	MIOS32_DOUT_SRSet(mm_hwcfg_ledrings.cathodes_ledrings_sr-1, select_mask);
      if( mm_hwcfg_ledrings.cathodes_meters_sr )
	MIOS32_DOUT_SRSet(mm_hwcfg_ledrings.cathodes_meters_sr-1, 0xff);
      ...
      // select the cathode of the meter (set the appr. pin to 0, and all others to 1)
      if( mm_hwcfg_ledrings.cathodes_ledrings_sr )
	MIOS32_DOUT_SRSet(mm_hwcfg_ledrings.cathodes_ledrings_sr-1, 0xff);
      if( mm_hwcfg_ledrings.cathodes_meters_sr )
	MIOS32_DOUT_SRSet(mm_hwcfg_ledrings.cathodes_meters_sr-1, select_mask);
  ...
  if( mm_hwcfg_ledrings.anodes_sr1 )
    MIOS32_DOUT_SRSet(mm_hwcfg_ledrings.anodes_sr1-1, anode_pattern & 0xff);
  if( mm_hwcfg_ledrings.anodes_sr2 )
    MIOS32_DOUT_SRSet(mm_hwcfg_ledrings.anodes_sr2-1, (anode_pattern >> 8) & 0xff);
